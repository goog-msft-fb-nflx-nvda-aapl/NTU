...
class ILQRController:
    ...
    def plan(self, x0: np.ndarray, U_init: np.ndarray) -> Tuple[np.ndarray, np.ndarray, List[np.ndarray], List[float], float]:
        ...
        for iter_num in range(self.max_iter):
            ...
            # 4) forward pass with line search
            X_new, U_new, J_new, accepted, alpha_used = self.forward_pass(x0, X, U, K_seq, k_seq, J)
            
            # Record line search statistics
            self.line_search_stats.append({
                'iteration': iter_num,
                'J_old': J,
                'J_new': J_new,
                'alpha': alpha_used,
    ...
    def forward_pass(
        ...
        if not self.use_line_search:
            # Original implementation without line search (alpha = 1.0)
            ...
            return X_new, U_new, J_new, True, 1.0
        ...
        for alpha in self.line_search_alphas:
            ...
            for k in range(N):
                # Apply step size alpha to both feedforward and feedback terms
                u = float(U[k, 0] + alpha * k_seq[k] + alpha * to_scalar(K_seq[k] @ to_vec(X_new[k] - X[k])))
                u = self._clip_u(u)
                X_new[k + 1] = self.f_discrete(X_new[k], u, self.dt)
                U_new[k, 0] = u
                J_new += self.stage_cost(X_new[k], u)
            ...
            if J_new < J_old:
                ...
                best_U = U_new
                best_J = J_new
                break  # Accept first improvement (Armijo-style)
            
            # Keep track of best result even if no improvement
            if J_new < best_J:
                best_alpha = alpha
                best_X = X_new
                best_U = U_new
        ...
        # If no improvement found, return best attempt
        if best_J < J_old:
            return best_X, best_U, best_J, True, best_alpha
        else:
            # No improvement, but return the best we found
            return best_X, best_U, best_J, False, best_alpha
    ...
    def print_line_search_stats(self):
        """Print line search statistics for debugging/analysis"""
        if not self.line_search_stats:
            print("No line search statistics available")
            return
        
        print("\n=== Line Search Statistics ===")
        for stat in self.line_search_stats:
            print(f"Iter {stat['iteration']:2d}: J_old={stat['J_old']:10.4f} -> J_new={stat['J_new']:10.4f}, "
                  f"alpha={stat['alpha']:.3f}, improvement={stat['improvement']:10.4f}, "
