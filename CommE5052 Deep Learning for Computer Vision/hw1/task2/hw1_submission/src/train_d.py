...
class PPM(nn.Module):
    ...
    def forward(self, x):
        size = x.shape[-2:]
        out  = [x] + [
            nn.functional.interpolate(s(x), size=size, mode='bilinear', align_corners=False)
...
class PSPNet(nn.Module):
    def __init__(self, n_classes=7):
        ...
        # dilate layer3 & layer4 to keep /8 resolution
        ...
        self.ppm  = PPM(2048, 512, pool_sizes=(1, 2, 3, 6))

        # auxiliary head (deep supervision on layer3)
    ...
    def forward(self, x):
        ...
        main = nn.functional.interpolate(self.head(x),    size=size, mode='bilinear', align_corners=False)
        if self.training:
            aux  = nn.functional.interpolate(self.aux_head(aux), size=size, mode='bilinear', align_corners=False)
            return main, aux
        return main
...
def train(args):
    ...
    model = PSPNet(n_classes=7).to(device)
    ...
    decoder_params  = list(model.ppm.parameters()) + list(model.head.parameters()) + \
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch == args.epochs // 2 or epoch == args.epochs:
            torch.save(model.state_dict(),
                       os.path.join(args.ckpt_dir, f'pspnet_epoch{epoch:03d}.pth'))

        if miou > best_miou:
            best_miou = miou
            torch.save(model.state_dict(),
                       os.path.join(args.ckpt_dir, 'pspnet_best.pth'))
