...
def run(args):
    ...
    # DeepLabV3+
    m = DeepLabV3Plus(n_classes=7).to(device)
    m.load_state_dict(torch.load(args.ckpt_c, map_location=device))
    m.eval(); models_list.append(('DeepLabV3+', m))
    print(f'Loaded DeepLabV3+  : {args.ckpt_c}')

    # PSPNet 60ep
    m = PSPNet(n_classes=7).to(device)
    m.load_state_dict(torch.load(args.ckpt_d1, map_location=device))
    m.eval(); models_list.append(('PSPNet-60ep', m))
    print(f'Loaded PSPNet-60ep : {args.ckpt_d1}')

    # PSPNet 100ep
    m = PSPNet(n_classes=7).to(device)
    m.load_state_dict(torch.load(args.ckpt_d2, map_location=device))
    m.eval(); models_list.append(('PSPNet-100ep', m))
    print(f'Loaded PSPNet-100ep: {args.ckpt_d2}')
    ...
    with torch.no_grad():
        for fname in tqdm(img_files):
            ...
            # weighted average: deeplab=1, psp60=1, psp100=0.8 (slightly lower weight since weaker)
            probs = (tta_probs(models_list[0][1], img_t, device) * 1.0 +
                     tta_probs(models_list[1][1], img_t, device) * 1.0 +
                     tta_probs(models_list[2][1], img_t, device) * 0.8) / 2.8
            ...
            if has_gt:
                gt = mask_to_label(np.array(Image.open(
                    os.path.join(args.input_dir, fname.replace('_sat.jpg','_mask.png'))).convert('RGB')))
                all_pred.append(pred); all_true.append(gt)

    if has_gt:
        miou = mean_iou(np.stack(all_pred), np.stack(all_true))
        print(f'\n3-model ensemble val mIoU: {miou:.4f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_d1', default='../checkpoints_psp/pspnet_best.pth')
    parser.add_argument('--ckpt_d2', default='../checkpoints_psp2/pspnet_best.pth')
