...
class PPM(nn.Module):
    ...
    def forward(self, x):
        size = x.shape[-2:]
        out = [x] + [nn.functional.interpolate(s(x), size=size, mode='bilinear', align_corners=False)
...
def main():
    ...
    parser.add_argument('input_dir',  help='directory with *_sat.jpg test images')
    parser.add_argument('output_dir', help='directory to write *_mask.png predictions')
    parser.add_argument('--ckpt_deeplab', default='checkpoints/deeplab101_best.pth')
    parser.add_argument('--ckpt_psp',     default='checkpoints_psp/pspnet_best.pth')
    ...
    model_c.load_state_dict(torch.load(args.ckpt_deeplab, map_location=device))
    model_c.eval()
    print(f'Loaded DeepLabV3+: {args.ckpt_deeplab}')

    # load PSPNet
    model_d = PSPNet(n_classes=7).to(device)
    model_d.load_state_dict(torch.load(args.ckpt_psp, map_location=device))
    model_d.eval()
    print(f'Loaded PSPNet    : {args.ckpt_psp}')
    ...
    with torch.no_grad():
        for fname in tqdm(img_files):
            ...
            Image.fromarray(label_to_mask(pred)).save(
