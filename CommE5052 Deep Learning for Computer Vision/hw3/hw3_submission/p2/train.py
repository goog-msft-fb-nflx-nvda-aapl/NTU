...
def train():
    ...
    save_dir = os.path.join(base_dir, 'p2', 'checkpoints')
    if is_main:
    ...
    if is_main:
        ...
        assert n_params < 10_000_000, f"Too many trainable params: {n_params}"
    ...
    for epoch in range(num_epochs):
        ...
        # Save checkpoint (only trainable params, only on main process)
        if is_main:
