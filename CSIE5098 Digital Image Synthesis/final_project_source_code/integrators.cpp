...
// PPMIntegrator Method Definitions
...
struct PPMPixelListNode {
    PPMPixel *pixel;
    PPMPixelListNode *next;
};

void PPMIntegrator::Render() {
    int nIterations = samplerPrototype.SamplesPerPixel();
    ProgressReporter progress(2 * nIterations, "Rendering", Options->quiet);
    const Float invSqrtSPP = 1.f / std::sqrt(nIterations);
    Film film = camera.GetFilm();
    ...
    CHECK(!pixelBounds.IsEmpty());
    Array2D<PPMPixel> pixels(pixelBounds);
    for (PPMPixel &p : pixels)
        p.radius = initialSearchRadius;
    ...
    for (int iter = 0; iter < nIterations; ++iter) {
        // --- Camera Pass: generate visible points ---
        Float uLambda = Options->disableWavelengthJitter ? Float(0.5)
                                                         : (iter + 0.5f) / nIterations;
        const SampledWavelengths passLambda = film.SampleWavelengths(uLambda);
        Float timeSample = (iter + 0.5f) / nIterations;
                ...
                while (true) {
                    ...
                    if (!si) {
                        for (const auto &light : infiniteLights) {
                            SampledSpectrum Le = light.Le(ray, lambda);
                            if (depth == 0 || specularBounce)
                                pixel.Ld += film.ToOutputRGB(beta * Le, lambda);
                        }
                        break;
                    }
                    SurfaceInteraction &isect = si->intr;
                    ...
                    // Direct lighting
                    SampledSpectrum Ld =
                        this->SampleLd(isect, bsdf, lambda, sampler, &lightSampler);
                    if (Ld)
                        pixel.Ld += film.ToOutputRGB(beta * Ld, lambda);
        ...
        // --- Build spatial grid over visible points ---
        int hashSize = NextPrime(nPixels);
        std::vector<std::atomic<PPMPixelListNode *>> grid(hashSize);

        Bounds3f gridBounds;
        Float maxRadius = 0;
        for (const PPMPixel &pixel : pixels) {
            if (!pixel.vp.beta)
                continue;
            Bounds3f vpBound = Expand(Bounds3f(pixel.vp.p), pixel.radius);
            gridBounds = Union(gridBounds, vpBound);
            ...
            for (Point2i pPixel : tileBounds) {
                ...
                for (int z = pMin.z; z <= pMax.z; ++z)
                    for (int y = pMin.y; y <= pMax.y; ++y)
                        for (int x = pMin.x; x <= pMax.x; ++x) {
                            int h = Hash(Point3i(x, y, z)) % hashSize;
                            PPMPixelListNode *node =
                                scratchBuffer.Alloc<PPMPixelListNode>();
                            node->pixel = &pixel;
                            node->next = grid[h];
                            while (!grid[h].compare_exchange_weak(node->next, node))
                                ;
        ...
        // --- Photon Pass ---
        ThreadLocal<ScratchBuffer> photonScratchBuffers(
            []() { return ScratchBuffer(); });

        ParallelFor(0, photonsPerIteration, [&](int64_t start, int64_t end) {
            ScratchBuffer &scratchBuffer = photonScratchBuffers.Get();
            Sampler sampler = threadSamplers.Get();
            for (int64_t photonIndex = start; photonIndex < end; ++photonIndex) {
                // Use per-iteration seed for basic PPM (non-Halton)
                uint64_t seed = (uint64_t)iter * photonsPerIteration + photonIndex;
                sampler.StartPixelSample({0, 0}, (int)(seed & 0x7fffffff));

                Float ul = sampler.Get1D();
                pstd::optional<SampledLight> sampledLight =
                    shootLightSampler.Sample(ul);
                if (!sampledLight)
                    continue;
                Light light = sampledLight->light;
                Float p_l = sampledLight->p;

                Point2f uLight0 = sampler.Get2D();
                Point2f uLight1 = sampler.Get2D();
                Float uLightTime = camera.SampleTime(timeSample);
                ...
                RayDifferential photonRay(les->ray);
                SampledSpectrum beta =
                    (les->AbsCosTheta(photonRay.d) * les->L) /
                    (p_l * les->pdfPos * les->pdfDir);
                if (!beta)
                    continue;
                ...
                for (int depth = 0; depth < maxDepth; ++depth) {
                    ...
                    if (depth > 0) {
                        ...
                        if (ToGrid(isect.p(), gridBounds, gridRes, &photonGridIndex)) {
                            int h = Hash(photonGridIndex) % hashSize;
                            for (PPMPixelListNode *node =
                                     grid[h].load(std::memory_order_relaxed);
                                 node; node = node->next) {
                                PPMPixel &pixel = *node->pixel;
                                if (DistanceSquared(pixel.vp.p, isect.p()) >
                                    Sqr(pixel.radius))
                                    continue;
                                Vector3f wi = -photonRay.d;
                                ...
                                if (pixel.vp.secondaryLambdaTerminated)
                                    photonLambda.TerminateSecondary();
                                RGB Phi_i = film.ToOutputRGB(
                                    pixel.vp.beta * Phi, photonLambda);
                                for (int i = 0; i < 3; ++i)
                                    pixel.Phi_i[i].Add(Phi_i[i]);
                                ++pixel.m;
                            }
                    ...
                    Vector3f wo = -photonRay.d;
                    pstd::optional<BSDFSample> bs = photonBSDF.Sample_f(
                        wo, sampler.Get1D(), sampler.Get2D(),
        ...
        threadScratchBuffers.ForAll(
            [](ScratchBuffer &buffer) { buffer.Reset(); });
        progress.Update();

        // --- Update radius and tau (PPM radius reduction with alpha) ---
        ParallelFor2D(pixelBounds, [&](Point2i pPixel) {
            PPMPixel &p = pixels[pPixel];
            if (int m = p.m.load(std::memory_order_relaxed); m > 0) {
                Float nNew = p.n + alpha * m;
                Float rNew = p.radius * std::sqrt(nNew / (p.n + m));
                RGB Phi_i(p.Phi_i[0], p.Phi_i[1], p.Phi_i[2]);
                p.tau = (p.tau + Phi_i) * Sqr(rNew) / Sqr(p.radius);
                p.n = nNew;
        ...
        if (iter + 1 == nIterations || (iter + 1 <= 64 && IsPowerOf2(iter + 1)) ||
            ...
            Image rgbImage(PixelFormat::Float, Point2i(pixelBounds.Diagonal()),
                           {"R", "G", "B"});
            ParallelFor2D(pixelBounds, [&](Point2i pPixel) {
                const PPMPixel &pixel = pixels[pPixel];
...
SampledSpectrum PPMIntegrator::SampleLd(const SurfaceInteraction &intr, const BSDF &b,
                                        SampledWavelengths &lambda, Sampler sampler,
                                        LightSampler lightSampler) const {
    const BSDF *bsdf = &b;
    LightSampleContext ctx(intr);
...
std::string PPMIntegrator::ToString() const {
    return StringPrintf("[ PPMIntegrator camera: %s initialSearchRadius: %f "
                        "maxDepth: %d photonsPerIteration: %d alpha: %f ]",
                        camera, initialSearchRadius, maxDepth, photonsPerIteration,
                        alpha);
}

std::unique_ptr<PPMIntegrator> PPMIntegrator::Create(
    const ParameterDictionary &parameters, const RGBColorSpace *colorSpace,
    Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights,
    const FileLoc *loc) {
    int maxDepth = parameters.GetOneInt("maxdepth", 5);
    int photonsPerIter = parameters.GetOneInt("photonsperiteration", -1);
    Float radius = parameters.GetOneFloat("radius", 1.f);
    Float alpha = parameters.GetOneFloat("alpha", 0.7f);
    return std::make_unique<PPMIntegrator>(camera, sampler, aggregate, lights,
                                           photonsPerIter, maxDepth, radius, alpha,
                                           colorSpace);
}
...
std::unique_ptr<Integrator> Integrator::Create(
    ...
    if (name == "path")
        ...
        integrator = PPMIntegrator::Create(parameters, colorSpace, camera, sampler,
                                           aggregate, lights, loc);
    else
