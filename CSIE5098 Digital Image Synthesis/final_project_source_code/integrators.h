...
class PPMIntegrator : public Integrator {
  public:
    ...
    static std::unique_ptr<PPMIntegrator> Create(const ParameterDictionary &parameters,
                                                 const RGBColorSpace *colorSpace,
                                                 Camera camera, Sampler sampler,
                                                 Primitive aggregate,
                                                 std::vector<Light> lights,
  ...
  private:
    ...
    int maxDepth;
    int photonsPerIteration;
    Float alpha;  // radius reduction parameter (typically 0.7)
    const RGBColorSpace *colorSpace;
    SampledSpectrum SampleLd(const SurfaceInteraction &intr, const BSDF &bsdf,
                             SampledWavelengths &lambda, Sampler sampler,
                             LightSampler lightSampler) const;
