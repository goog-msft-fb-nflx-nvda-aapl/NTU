...
// StructuredInfiniteLight Definition
class StructuredInfiniteLight : public LightBase {
  public:
    StructuredInfiniteLight(Transform renderFromLight, Image image,
                            const RGBColorSpace *imageColorSpace, Float scale,
                            std::string filename, int nStrata, Allocator alloc);
    ...
    pstd::optional<LightLiSample> SampleLi(LightSampleContext ctx, Point2f u,
        ...
        // Pick a stratum uniformly
        int si = std::min<int>(u[0] * strata.size(), strata.size() - 1);
        // Re-use u[0] within the stratum
        Float ru = u[0] * strata.size() - si;
        const Bounds2f &b = strata[si];

        // Sample a point uniformly within this stratum
        ...
        // PDF: uniform over strata, uniform within stratum
        // Float stratumArea = (b.pMax.x - b.pMin.x) * (b.pMax.y - b.pMin.y);
        // Float mapPDF = 1.f / (strata.size() * stratumArea);
        // Float pdf = mapPDF / (4 * Pi);
        // Float pdf = 1.f / (4 * Pi);
        Float stratumArea = (b.pMax.x - b.pMin.x) * (b.pMax.y - b.pMin.y);
  ...
  private:
    ...
    // Build strata by recursively splitting the brightest region
    void BuildStrata(const Array2D<Float> &d, int nStrata, Allocator alloc);
